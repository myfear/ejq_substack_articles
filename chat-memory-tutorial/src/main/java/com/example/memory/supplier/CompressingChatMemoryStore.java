...
public class CompressingChatMemoryStore implements ChatMemoryStore {
    ...
    public CompressingChatMemoryStore(ChatModel model, // We use the default chat model, but you can select any chat
                                                       // model
    ...
    public void updateMessages(Object memoryId, List<ChatMessage> messages) {
        // Extract the last message if any, as we do not want to compress during
        // function calls
        if (messages.isEmpty()) {
        ...
        // Also skip compression if the last message is a system message or a function
        // call response
        if (lastMessage.type() == ChatMessageType.SYSTEM
                || lastMessage.type() == ChatMessageType.TOOL_EXECUTION_RESULT) {
            Log.infof(
        ...
        if (messages.size() > threshold) {
            ...
            // compressed now contains a "fake" system message with the previous summary if
            // it existed, and all other messages.
    ...
    private void extractSummaryFromSystemMessageIfAny(SystemMessage systemMsg, List<ChatMessage> compressed) {
        ...
        // Otherwise, do nothing, as we don't want to include the system message in the
        // compressed messages.
    }
